...
class PostDetailRepositoryImpl @Inject constructor(
    ...
    suspend fun incrementCommentViewCount(commentId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            client.postgrest.rpc("increment_comment_views", mapOf("comment_id" to commentId))
            Result.success(Unit)
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
            Log.e(TAG, "Failed to increment comment view count: ${e.message}", e)
            Result.success(Unit)
        }
    }
    ...
    private fun parsePostFromJson(data: JsonObject): Post {
        val post = Post(
            ...
            postHideReplyCount = data["post_hide_comments_count"]?.let { if (it is kotlinx.serialization.json.JsonPrimitive) it else null }?.contentOrNull,
            postDisableReplies = data["post_disable_comments"]?.let { if (it is kotlinx.serialization.json.JsonPrimitive) it else null }?.contentOrNull,
            ...
            replyCount = data["comments_count"]?.let { if (it is kotlinx.serialization.json.JsonPrimitive) it else null }?.intOrNull ?: 0,
            ...
            inReplyToPostId = data["in_reply_to_post_id"]?.jsonPrimitive?.contentOrNull,
            rootPostId = data["root_post_id"]?.jsonPrimitive?.contentOrNull
        )
